...
/**
 * The LibraryManagement class has multiple methods that is the backbone to this program. It also loads the books.txt into 
 * the arraylist. 
...
public class LibraryManagements {
	...
	/**
	 * Use this to test if books.txt is being scanned properly. It prints to the screen if successful.
	 * Only use it for testing and debugging purpose. 
	 * 
	 * @param libraryList
	 * @throws IOException
	 */
	...
	public static void checkOut(ArrayList<Book> bookList) {
		...
		while(pass1 != true) {
			...
			try {
				...
				//for loop to iterate through the list and compare if input ISBN exists. If exists, get available if greater than 0. 
				for (int i = 0; i<bookList.size(); i++) {
					...
					//if ISBN has an availability of 0, it will tell the user the book does not exist. 
					if (bookList.get(i).getISBN() == bookISBN && bookList.get(i).getAvailable() == 0) {
	...
	public static void produceRandomBook(ArrayList<Book> libraryList) {
		...
		//while loop just in case the user gets an error, it repeats the process for the user to enter an input again. 
		while (pass4 != true) {

			//Error catch. If the user input creates an error, it will catch it and print out invalid input.  
			try {
	...
	public static void findBooksByTitle(ArrayList<Book> bookList) {
		...
		//while loop just incase the user enters an error. It repeats the process for the user to input again. 
		while (pass2 != true) {
			...
			try {	
				...
				//Iterates through the book list. If the list contains the input, convert it to lower case. This makes it case insensitive. 
				for (int i = 0; i<bookList.size(); i++ ) {
				...
				//If the user input does not match anything, the boolean turns false and it will print out that the title does not exist. 
				if (exist2 == false) {
	...
	public static void booksByType(ArrayList<Book> libraryList) {
		...
		while (pass3 != true) {
			...
			//Catches the error in case the user does a wrong input. Prints a message saying invalid input. 
			try {
				System.out.println();
	...
	public static void loadLibrary(ArrayList<Book> libraryList) throws IOException {
		...
		while (input.hasNextLine()) {
			...
			if (modISBN == 0 || modISBN == 1) {
				...
				libraryList.add(new ChildrenBooks(getISBN, getCall, getAvailable, getTotal, getTitle, getAuthor, getFormat));
			...
			if (modISBN == 2 || modISBN == 3) {
				...
				getDiet = bookArray[6];
				libraryList.add(new CookBooks(getISBN, getCall, getAvailable, getTotal, getTitle, getPublisher, getDiet));
				//Arrays.fill(bookArray, null);
			}
			...
			if (modISBN >= 4 && modISBN <=7) {
				...
				libraryList.add(new PaperBacks(getISBN, getCall, getAvailable, getTotal, getTitle, getAuthor, getYear, getGenre));
				//Arrays.fill(bookArray, null);
			}
